...
// net/http or golang.org/x/net/http.Http2
package h2c
...
// convert it to an HTTP/2 connection and pass the net.Conn to http.Http2ServeConn.
...
func NewHandler(h http.Handler, s *http.Http2Server) http.Handler {
	return &h2cHandler{
		Handler: h,
		s:       s,
	}
...
func (s h2cHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PRI" && len(r.Header) == 0 && r.URL.Path == "*" && r.Proto == "HTTP/2.0" {
		...
		defer conn.Close()
		s.s.ServeConn(conn, &http.Http2ServeConnOpts{
			Context:          r.Context(),
...
// knowledge (Section 3.4) and creates a net.Conn suitable for http.Http2ServeConn.
